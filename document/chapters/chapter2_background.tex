...
Zstandard is a lossless compression algorithm developed by Facebook. It is based on the DEFLATE algorithm, but it is much faster and has a better compression ratio. It is also a very popular algorithm, being used by Meta, Google, Amazon and Apple.

Gzip is also based on DEFLATE, it was created as an alternative to the compress program in Unix systems, because of the Unisys and IBM patens covering the LZW algorithm.

Bzip2 is a lossless compression algorithm based on the Burrows-Wheeler transform. It is more efficient than DEFLATE but is also slower. Even though it can be used in any type of data it is most efficient in text based data.
...
Cmix is a lossless compression algorithm that uses several machine learning models to predict the next symbol in a sequence. It is a very efficient algorithm, however it is extremely cpu and memory intensive, requiring a minimum of 32GB of memory to run it.
...
Paq1 is a context mixing compression algorithm. It uses an arithemetic encoder that uses the weighted average of 5 bit-level predictors, to predict the next symbol in a sequence.
